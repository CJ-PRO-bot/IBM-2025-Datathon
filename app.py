...
# ---------------------------------
# Project paths & environment config
# ---------------------------------
ROOT = Path(__file__).resolve().parent

def _abs(p: str | None) -> str | None:
    """Return absolute path; if relative, make it relative to project ROOT."""
    if not p:
        return None
    path = Path(p)
    return str(path if path.is_absolute() else (ROOT / path))

PV_MODEL_PATH     = _abs(os.getenv("PV_MODEL_PATH", "ai/waste_v1/validity_classifier.onnx"))
PV_CLASS_MAP_PATH = _abs(os.getenv("PV_CLASS_MAP_PATH", "ai/waste_v1/class_map.json"))
PV_MODEL_VERSION  = os.getenv("PV_MODEL_VERSION", "waste_v1_onnx")
PV_ACTION_CUTOFF  = float(os.getenv("PV_ACTION_CUTOFF", "0.50"))
PV_DISABLE_DUP_PENALTY = os.getenv("PV_DISABLE_DUP_PENALTY", "1") == "1"
PV_DUP_DISTANCE   = int(os.getenv("PV_DUP_DISTANCE", "3"))

# Make sure downstream code (e.g., ai.verifier) can see these values via os.environ.
os.environ["PV_MODEL_PATH"] = PV_MODEL_PATH or ""
os.environ["PV_CLASS_MAP_PATH"] = PV_CLASS_MAP_PATH or ""
os.environ["PV_MODEL_VERSION"] = PV_MODEL_VERSION
os.environ["PV_ACTION_CUTOFF"] = str(PV_ACTION_CUTOFF)
os.environ["PV_DISABLE_DUP_PENALTY"] = "1" if PV_DISABLE_DUP_PENALTY else "0"
os.environ["PV_DUP_DISTANCE"] = str(PV_DUP_DISTANCE)
...
# ----------------------------
app = Flask(__name__)
app.config["SECRET_KEY"] = "change-me"  # TODO: put a real secret in PROD
...
def index():
    if request.method == "POST":
        ...
        # --- Use pluggable verifier (pHash + EXIF + model/ONNX or heuristic)
